...
export default {
  ...
  methods: {
    ...
    async createMP4() {
      ...
      this.store.setOutputDate(this.getAnimationDateTitle(this.mapTimeSettings.Step));
    },
    async createMP4Handler() {
      ...
      for (
        ...
        if (this.generating === true) {
          ...
          this.store.setMP4Percent(Math.round((progressCounter / this.MP4Length) * 100));
        }
      }
    ...
    getInfoCanvas(visibleLayers, widths, customTitle) {
      ...
      fontArray.forEach((timeLayer) => {
        ...
        if (
          this.colorBorder &&
          this.activeLegends.includes(timeLayer.name)
        ) {
  ...
  computed: {
    animationTitle(){return this.store.getAnimationTitle},
    datetimeRangeSlider(){return this.store.getDatetimeRangeSlider},
    framesPerSecond(){return this.store.getFramesPerSecond},
    isAnimating(){return this.store.getIsAnimating},
    isAnimationReversed(){return this.store.getIsAnimationReversed},
    MP4ProgressPercent(){return this.store.getMP4ProgressPercent},
    pendingErrorResolution(){return this.store.getPendingErrorResolution},
    playState(){return this.store.getPlayState},

    activeLegends(){return this.store.getActiveLegends},
    colorBorder(){return this.store.getColorBorder},
    currentAspect(){return this.store.getCurrentAspect},
    currentResolution(){return this.store.getCurrentResolution},
    imgURL(){return this.store.getImgURL},
    mapTimeSettings(){return this.store.getMapTimeSettings},
    mp4URL(){return this.store.getMP4URL},
    outputDate(){return this.store.getOutputDate},
    outputFormat(){return this.store.getOutputFormat},
    layersVisible() {
      let visibleLayers = this.$mapLayers.arr.filter((l) => {
        return l.get("layerVisibilityOn") && l instanceof OLImage;
      });
